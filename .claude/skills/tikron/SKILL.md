...
- **`CasualRealtimeRoom`** — free movement but NO physics tick (cursors, whiteboards,
  ...
  try/catch yourself).
- **`IoArenaRoom`** — fixed-timestep simulation (`.io` arenas, FPS, racers).
  ...
  `inputBatchMs`). Reference: `apps/ironsight`'s `ArenaRoomImpl`
  (`src/rooms/arena-room.ts`). Details in AGENTS.md →
  "Competitive FPS".
...
(`CasualRealtimeRoom`) as a working reference; `apps/ironsight`'s `ArenaRoomImpl` shows
...
(not for a twitch FPS), rooms fixed near their creator (place with a matchmake `region`
