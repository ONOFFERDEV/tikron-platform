...
Synty 캐릭터 20종은 같은 스켈레톤(Epic 계열 본명)을 공유한다. UAL(Universal Animation
Library) 전 클립을 그 스켈레톤에 리타깃한 공용 라이브러리 GLB(`synty-anim-library.glb` —
스켈레톤+클립만, 메시 없거나 최소)를 얹으면 클립 드롭다운에 120+ 클립이 전부 떠야 한다.

16. **UI**: LOAD 패널에 "Load animation library…" 보조 버튼(전용 파일 입력, 항상 라이브러리로
    취급 — 모호함 없음). 기존 드롭존/"Choose file…"에 GLB를 놓았을 때 **이미 모델이 로드된
    상태면** "모델 교체 vs 애니메이션 라이브러리로 추가" confirm으로 분기.
17. **병합 동작**: 두 번째 GLB에서 `AnimationClip`만 추출(본 계층/메시는 버림)해 현재
    `rig.clips` 배열에 병합. 본 이름 매칭 — three.js `AnimationMixer`가 트랙 이름
    (`"본명.quaternion"` 등)으로 현재 모델의 노드 그래프를 검색해 바인딩하므로, 원본 GLB가
    달라도 본명만 같으면 그대로 재생된다(재바인딩 로직 불필요). 라이브러리 클립이 참조하는
    고유 본명 중 현재 모델에 없는 비율이 30%를 넘으면 로드 전 경고 confirm. 클립 이름이
    기존과 충돌하면 교체 여부를 confirm(교체 거부 시 해당 클립은 스킵 — 동명 클립 두 개가
    공존하는 상태는 절대 만들지 않는다: `editedKeys`가 `클립명::본명`으로 키잉되므로 동명
    중복은 키 편집 데이터가 섞이는 버그가 된다).
18. 클립 드롭다운·타임라인·키프레임 편집·전역 보정은 `rig.clips`가 평범한 배열이라 병합
    클립에도 코드 변경 없이 그대로 작동(스모크로 실제 재생까지 확인 — 확인만 하고 끝내지
    않는다).
19. **선택 내보내기**: Baked GLB export가 다이얼로그로 바뀐다 — 클립 체크리스트(디폴트=
    idle/walk/run/death 중 존재하는 것만 체크, 그 4종이 하나도 없으면 전체 체크) → 체크된
    클립만 굽는다. 120클립 전부 구우면 파일이 커지는 것을 막기 위함.
20. 테스트 훅(`__rigEditorTest`)의 `getState()`에 `clipNames: string[]` 노출(라이브러리 병합
    후 클립 목록을 자동화 테스트가 검증할 수 있도록).
