...
describe("Matchmaker REST", () => {
  ...
  it("mid-join at scale: 64 players fill one room, the 65th opens a new one", async () => {
    // A 64-seat game connects with max=64. reserve() must pack players into the
    // same room until it is full (mid-join is the whole point), then spill to a
    // fresh room — proving max=64 flows through to the room cap.
    const first = await api("/api/matchmake?type=t-midjoin&max=64");
    for (let i = 1; i < 64; i++) {
      const next = await api("/api/matchmake?type=t-midjoin&max=64");
      expect(next.roomId).toBe(first.roomId); // still space -> same room (mid-join)
    }
    const full = await api("/api/rooms?type=t-midjoin");
    const room = full.find((r: any) => r.roomId === first.roomId);
    expect(room.count).toBe(64);
    expect(room.maxClients).toBe(64);
    expect(room.locked).toBe(true);

    const overflow = await api("/api/matchmake?type=t-midjoin&max=64");
