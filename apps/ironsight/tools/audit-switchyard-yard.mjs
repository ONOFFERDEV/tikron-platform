...
const result = await build({ stdin: { contents: `
  ...
  export { SWITCHYARD_YARD_PARTS as parts, SWITCHYARD_YARD_CRATES as crates,
    SWITCHYARD_YARD_OLD_BLOCKS as oldBlocks } from './src/map/switchyard-yard.js';
  export { buildSwitchyardEnvironment } from './client/switchyard-environment.js';
...
const { map, parts, crates, oldBlocks, buildSwitchyardEnvironment, PROP_LIBRARY, Scene, Matrix4,
...
const scene = new Scene(); buildSwitchyardEnvironment(scene,map,true); const shells=[];
...
// boundary and check the union, rather than requiring one enclosing box.
const supports=map.boxes.filter(b=>b.max.y===0);
for(const p of paint) {
  const axes=['x','z'].map(k=>[p.min[k],p.max[k],...supports.flatMap(b=>[b.min[k],b.max[k]])
    .filter(v=>v>p.min[k]+1e-5&&v<p.max[k]-1e-5)].sort((a,b)=>a-b));
  for(let i=1;i<axes[0].length;i++)for(let j=1;j<axes[1].length;j++) {
    const x=(axes[0][i-1]+axes[0][i])/2,z=(axes[1][j-1]+axes[1][j])/2;
    assert.ok(supports.some(b=>b.min.x<=x+1e-5&&b.max.x>=x-1e-5&&b.min.z<=z+1e-5&&b.max.z>=z-1e-5),
      'unsupported ground mark: '+JSON.stringify({p,x,z}));
  }
}
checks.push(`${paint.length} rendered ground marks have complete ground/bridge support; none hangs over the rail cut`);
const route = [[46,46],[46,42],[47,40],[47,35],[47,37.6],[42,37.6],[41.6,38],[41.6,42],[46,42],[52.4,42],[55,42],[52.4,42],[52.4,37.6],[47,37.6],[46,42],[46,46]];
let steps=0;
...
checks.push('walk/sprint both staggered entries and thin-wall circuits around switchgear benches');
for(const boxes of [collision.closedHits,collision.openHits]) for(const east of [false,true]) {
  const x=n=>east?150-n:n;
  assert.equal(nearestBox({x:x(47),y:1.65,z:35},{x:0,y:0,z:1},boxes,5),Infinity);
  assert.equal(nearestBox({x:x(46),y:1.65,z:46},{x:0,y:0,z:-1},boxes,4),Infinity);
  for(const xx of [42,51]) assert.ok(nearestBox({x:x(xx),y:1.65,z:35},{x:0,y:0,z:1},boxes,3)<3);
  assert.ok(nearestBox({x:x(50),y:.65,z:41},{x:0,y:0,z:-1},boxes,2)<2);
  assert.equal(nearestBox({x:x(50),y:1.65,z:41},{x:0,y:0,z:-1},boxes,2),Infinity);
}
...
let navSteps=0;
for(const spawn of [...map.spawns.red,...map.spawns.blue]) for(const goal of [{x:46,z:42},{x:104,z:42}]) {
...
await writeFile(process.argv[2]??'.inspect/session101-yard-audit.json',JSON.stringify(report,null,2));console.log(report);
