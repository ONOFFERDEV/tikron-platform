...
export function rifleSight(x: number, railY: number): { object: T.Group; centerY: number; geometry: T.BufferGeometry[] } {
  const object = new T.Group(), centerY = railY + 0.052;
  const parts = [
  ...
  const frame = mergeGeometries(parts)!; parts.forEach(p => p.dispose());
  object.add(new T.Mesh(frame, frameMaterial));
  ...
  return { object, centerY, geometry: [frame, dot] };
}
