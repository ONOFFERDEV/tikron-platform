...
export class SupportHud {
  ...
  private readonly pips = Array.from({ length: 3 }, () => document.createElement('i'));
  private current = emptySupport();
  private bannerUntil = 0;
  private readonly announced = new Set<string>();
  private pulseAt = 0;
  constructor(private readonly cue: (kind: 'earned' | 'friendly' | 'enemy' | 'pulse') => void,
    private readonly width: number, private readonly depth: number) {
  ...
  update(state: ArenaState, id: string, now: number, online: boolean, blackout: boolean, playing = true): SupportView {
    ...
    const label = friendly ? `UAV ONLINE / ${Math.ceil((friendly.endsAt - now) / 1000)}s` : queued ? 'UAV READY / HOLDING' : this.current.count >= RECON.kills ? 'UAV / DEPLOYED THIS LIFE' : `UAV / ${this.current.count} OF 3`;
    const detail = blackout && (friendly || queued) ? 'Signal lost. Scans blocked until relink.' : friendly ? 'Last-seen contacts. New scan every 4s.' : queued ? `Airspace clears in ${Math.max(0, Math.ceil((this.current.readyAt - now) / 1000))}s. Stay alive.` : this.current.count >= RECON.kills ? 'One UAV per life. Keep the streak going.' : state.mode === 3 ? 'Three eliminations without dying. Private recon rehearsal.' : 'Three eliminations without dying. Automatic team recon.';
    ...
    for (const [index, pip] of this.pips.entries()) pip.dataset.filled = String(this.current.count > index);
    return this.current;
  }
