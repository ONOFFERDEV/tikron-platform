...
export const RELAY_BRICK_WEAR = `
  if (vRelayWall > 0.5) {
    ...
    diffuseColor.rgb *= mix(vec3(0.84, 0.81, 0.78), vec3(1.08, 1.03, 0.95), tone);
    ...
    diffuseColor.rgb *= 1.0 - soot * 0.32;
    ...
    float lostRows = step(0.42, ragged) * (ragged - 0.42) * 9.0;
    float rowsDown = fromTop / brickSize.y + relayHash(brickCell + 3.7) * 1.3;
    float lost = step(1.0, faceExtent) * step(rowsDown, lostRows);
    diffuseColor.rgb = mix(diffuseColor.rgb, diffuseColor.rgb * vec3(0.46, 0.40, 0.34), lost);
    ...
    float scarRadius = 0.16 + 0.34 * relayHash(scarCell + 9.1);
    float scarDist = length((metres - scarCentre) * vec2(1.0, 1.2)) / scarRadius
      + (relayNoise(metres * 11.0) - 0.5) * 0.55;
    float scarOn = step(0.82, scarSeed) * step(0.35, fromBase);
    ...
    diffuseColor.rgb = mix(diffuseColor.rgb, diffuseColor.rgb * vec3(0.58, 0.52, 0.46), scarCore);
    diffuseColor.rgb = mix(diffuseColor.rgb, diffuseColor.rgb * vec3(1.10, 1.07, 1.00), scarRim * 0.5);
    ...
    float plugged = repairKey < 0.16 ? 1.0 : 0.0;
    float repairBottom = repairKey < 0.12 ? 0.25 : 0.0;
    ...
    if (repairKey < 0.07) {
      ...
      repairColour = vec3(0.13, 0.095, 0.06) * mix(0.75, 1.25, relayHash(vec2(board, segment) + faceKey));
      repairColour *= 1.0 - seam * 0.5;
      repairRelief = (1.0 - seam) * 0.008;
      roughnessFactor = mix(roughnessFactor, 0.9, inRepair);
    } else if (repairKey < 0.12) {
