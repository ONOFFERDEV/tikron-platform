...
### Assets stream Session 1 - reviewed field supply library (2026-09-11)

`client/prop-library.ts` exposes six original/generated props by stable name.
Importing it fetches nothing. Create one `createPropLibrary()` per map lifetime,
`await library.load(name)` during loading (before texture/shader warm-up), place
the returned independent Group in metres, then remove instances and call
`library.dispose()` on teardown. Copies share immutable geometry/materials;
clone a material before changing it. The root is Y-up with its base centred at
the origin, and each entry documents its width/height/depth and placement rule.
No lights or collision are created. Apparent cover must match the authoritative
map or remain wholly outside play. The assets stream does not place these props.

| New asset | Shipped bytes | Triangles | Metre envelope (X/Y/Z) | Images |
|---|---:|---:|---|---|
| `props/ammo-crate-stack.glb` | 238624 | 2840 | .531171 / 1.15 / .598316 | 3 x 512px WebP |
| `props/field-chest-panel.glb` | 229180 | 3550 | .510897 / .48 / .382378 | 2 x 512px WebP |

Both are Meshy-generated originals for this project under the account's Meshy
terms, with no purchased input, real model name, brand or unit insignia. Adjacent
`.meta.json` files carry full original prompts, preview/refine IDs, source and
shipped SHA256, measured dimensions, texture count, review verdict and exact
processing command. The audit checks shipped hashes, byte/triangle counts,
embedded WebP and the review receipt. `props/.gitignore` explicitly allowlists
reviewed GLBs; existing purchased-source ignore rules remain in effect.

The crate stack was generated by `tools/meshy-batch.mjs --concurrency 4 --only
ammo-crate-stack,field-generator --out .inspect/assets-session1-meshy`. Raw source
6,898,948 bytes; preview `01a08be2-47b2-70e5-a86b-dad222eef745`, refine
`01a08be3-82e5-77f1-8c6c-95f9b8e247a0`. The generator was rejected: broken
triangular framing, an unwanted wheel and weak cabinet silhouette. It is not
shipped. The account debit was **60 credits for the pair**, 1230 -> 1170. The
old parallel runner reported the overlapping 60-credit balance interval twice;
per-job cost is an estimated 30, never a second 60-credit charge. The corrected
receipt is `.inspect/assets-session1-credit-receipt.json`. The batch tool now
caps planned spend (default 60), keeps a reserve (default 300, minimum 200),
records one account receipt, serializes manifest writes and refuses accidental
resubmission over an existing model/task. `--dry-run` lists the budgeted queue
without accessing the account. Regeneration remains paid and nondeterministic.

The chest rig reuses the retained Session84 source, so it costs zero new credits.
Raw source 6,651,012 bytes; preview `01a08ac8-5f34-76b0-a82a-65c63e6e98c1`,
refine `01a08ac9-9177-7648-bd9e-cc079b713322`. Despite its source name, the
result is a **full hollow chest rig**, not the requested flat chest panel. It is
approved for static supply dressing only; do not attach it as a flat skinned
plate. The faceted generated normal map was omitted, saving one 512px texture;
original vertex normals and UVs remain. Pouches face +Z. The two retained
helmet variants and the long plate carrier were rejected after thumbnail and
front/side/back GLB review (folded surfaces, protrusions, distorted pouches).
The existing 260176-byte vertex-colour radio is reused without adding its older
PBR duplicate. Existing sandbag, uplink and transformer GLBs are unchanged.

Reproduce processing from the retained local sources with Blender 4.5:

```powershell
& 'C:/Program Files/Blender Foundation/Blender 4.5/blender.exe' --background --python tools/shrink-glb.py -- --input .inspect/assets-session1-meshy/ammo-crate-stack/model.glb --output public/assets/props/ammo-crate-stack.glb --size 512 --webp --height-m 1.15
& 'C:/Program Files/Blender Foundation/Blender 4.5/blender.exe' --background --python tools/shrink-glb.py -- --input .inspect/assets-session1-kit84/field-chest-panel/model.glb --output public/assets/props/field-chest-panel.glb --size 512 --webp --height-m .48 --omit-normal-map
node .inspect/inspect-prop-gallery.mjs --url http://localhost:8797
```

The gallery runner bundles `.inspect/prop-gallery.ts` and serves its review page
through CDP interception; nothing is added to the game entrypoint or public JS.
It records all props in one metre-scale row, a 2m original reference mannequin,
matched before/after supply views and three angles per prop. The JSON report
checks base pivots, metre bounds, no eager requests, cloned transforms, shared
buffers, disposed-loader rejection, load timings, texture residency and frames.
The two new GLBs add **467804 bytes** and **6.667 MiB estimated mipmapped RGBA
texture residency if both are loaded**. In the game they add zero requests or
resident textures until the owning map stream integrates them lazily.
