...
for (const clip of clips) {
  ...
  for (const channel of clip.channels) {
    ...
    if (name !== 'backpedal') assert.match(baked.json.nodes[channel.target.node].name,
      directional ? /^(clavicle_|UpperArm_|lowerarm_|Hand_|thumb_|indexFinger_|finger_|Thigh_|calf_|Foot_|ball_)/ : /^(clavicle_|UpperArm_|lowerarm_|Hand_|thumb_|indexFinger_|finger_)/);
  ...
  for (const channel of base.channels) {
    const bone = original.json.nodes[channel.target.node].name;
    const authored = /^(clavicle_|UpperArm_|lowerarm_|Hand_|thumb_|indexFinger_|finger_)/.test(bone)
      || directional && /^(Thigh_|calf_|Foot_|ball_)/.test(bone) || name === 'backpedal';
    if (authored && channel.target.path === 'rotation') continue;
    assert.ok(clip.channels.some(c => JSON.stringify(c) === JSON.stringify(channel)), `preserved ${bone} ${channel.target.path}`);
  }
}
