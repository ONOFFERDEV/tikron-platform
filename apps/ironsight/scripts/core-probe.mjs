...
export async function coreProbe({send,evaluate,delay,capture,waitFor,flood=false}) {
  const bundle=await build({stdin:{contents:`import { ${flood?'ARENA2':'ARENA1'} as ARENA1 } from './src/map/${flood?'arena2':'arena1'}.js';
  ...
  const entry=flood?65:67,exit=flood?85:83;
  ...
  try{
    await walk({x:entry,z:50});await evaluate('window.ironsight.look(Math.PI/2,.08)');
    ...
    const opened=await snapshot();const started=Date.now();await capture('core-opened');
    ...
    await walk({x:exit,z:50});await waitFor('!window.ironsight.state().coreOpen');
    await evaluate('window.ironsight.look(-Math.PI/2,.08)');
    const closed=await snapshot();await capture('core-sealed');
    if(before.open||!opened.open||opened.map!==(flood?'online':'offline')||!inside.open||inside.me.x<74||inside.me.x>76||
      !held.open||held.map!=='online'||!held.hud.includes('CLEAR TO SEAL')||closed.open||closed.me.x<exit-1||
    ...
    return{elapsedMs:Date.now()-started,before,opened,inside,held,closed,layout,note:'Ordinary training matchmaking; actual server schedule and collision-routed W. Crossed the passage, held both shutters by standing inside, exited and observed safe closure. No state/clock/position edits.'};
