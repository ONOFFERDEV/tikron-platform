...
export async function undertowYardProbe({ send, evaluate, delay, capture, record, east = false }) {
  ...
  const travel=async(goal,sprint=false,ground=false)=>{
    ...
    try {for(;;){
      ...
      if(!sample.me.alive||Date.now()>deadline)throw Error(`Undertow yard route stalled: ${JSON.stringify({p,goal})}`);
  ...
  try {
    await travel({x:34,z:84},false,true);
    ...
    for(const sprint of [false,true]) {
      ...
      await stage(`${mode}-bay-front`,{x:35,y:2,z:76},0);
      await path([[34,80],[35,78]],sprint);
      await stage(`${mode}-service-bay`,{x:29.5,y:1.4,z:78.75},0);
      await path([[35,75]],sprint);
      await stage(`${mode}-north-breach`,{x:35,y:1.65,z:82},0);await fire();
      await path([[35,77.5],[32,77.5],[26.6,77.5]],sprint);
      await stage(`${mode}-thin-wall`,{x:30,y:1.5,z:79},0);
      await path([[26.6,80.5],[34,80],[43.3,80]],sprint);
      await stage(`${mode}-equipment-cover`,{x:39,y:1.1,z:79},0);await fire();
      await path([[43.3,77.6],[35,77.6],[34,80],[34,84]],sprint);
      await stage(`${mode}-south-exit`,{x:29.5,y:2,z:81},0);
    }
