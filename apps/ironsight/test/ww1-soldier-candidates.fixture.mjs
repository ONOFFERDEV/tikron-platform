...
try {
  ...
  for (const endings of ['LF', 'CRLF', 'mixed']) {
    const publicRoot = await stage(endings);
    for (const asset of admission.assets) {
      const path = join(publicRoot, asset.meta), text = (await readFile(path, 'utf8')).replaceAll('\r\n', '\n');
      await writeFile(path, endings === 'CRLF' ? text.replaceAll('\n', '\r\n') : endings === 'mixed' ? text.replace('\n', '\r\n') : text);
    }
    const normalized = await auditWw1SoldierCandidates(publicRoot, builder);
    assert.equal(normalized.valid, false); assert.equal(normalized.files.length, 2);
    assert.equal(normalized.issues.some(issue => issue.code === 'soldier_candidate_quarantined'), true);
    assert.equal(normalized.issues.some(issue => issue.code === 'soldier_meta_hash'), false);
    assert.deepEqual(normalized.issues, baseline.issues);
  }
  ...
  const report = { verdict: 'PASS_QUARANTINED_STAGING16', files: baseline.files, scenarios: ['exact-quarantined-two-64-clips', 'LF-text', 'CRLF-text', 'mixed-text', 'changed-glb', 'changed-meta', 'forged-runtime-acceptance'] };
