...
describe.skipIf(process.env.SWITCHYARD_METRICS !== '1')('expanded Switchyard natural bot round', () => {
  it('records a complete twelve-player FFA round, life contact samples and a death heatmap', { timeout: 120000 }, async () => {
    ...
    try {
      ...
      writeFileSync('.inspect/session30-bot-debug.json', JSON.stringify({state,lives,killCount:kills.length},null,2));
      ...
      writeFileSync('.inspect/session30-bot-round.json', JSON.stringify(report, null, 2));
      ...
      writeFileSync('.inspect/session30-bot-heatmap.svg', `<svg xmlns="http://www.w3.org/2000/svg" viewBox="-2 -12 154 116"><rect x="-2" y="-12" width="154" height="116" fill="#132b33"/><text x="1" y="-5" fill="white" font-size="4">SWITCHYARD / 12 FFA bots / ${Math.max(...Object.values(state.players).map(p => p.k))} winning kills / ${((endedAt-liveAt)/1000).toFixed(1)}s</text>${solids}${heat}</svg>`);
